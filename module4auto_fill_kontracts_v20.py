...
+                                         Справка  (Раздел в разработке)                                    +
                       ...
                       что в свою очередь предполагает что + это согласие с чем-то, если
...
print("Перейдите по любой из следующих ссылок, для получения дополнительных реквизитов компании, т.к. на этих сайтах установлена защита от ботов и я пока не умею ее обходить.")
url_inn1 = f'https://'
url_inn2 = f"https://"
url_inn3 = f"https://"
print(url_inn1)
...
time.sleep(2)
try:
    zagr_url = f"https://zakupki.gov.ru/epz/order/notice/ea20/view/documents.html?regNumber={nomer_zakup}"
    r_zagr = rq.get(zagr_url, timeout=10, headers=headers)
    html_zagr = bs(r_zagr.content, "html.parser")
    athef4dwnldall = html_zagr.find("div", class_=re.compile("first-row-active-documents")).find_all("a", title=re.compile(""))

    for i in athef4dwnldall:
        i = f"""{i}"""
        if "zakupki.gov.ru/44fz/filestore/public/1.0/download/priz/file.html?uid" in i:
            athef4dwnld.append(i.split('"'))

    for i in athef4dwnld:
        slvr_zagr_url[nomer_dla_opredelenia_slovarnix_ssilok] = [i[3], i[1]]
        nomer_dla_opredelenia_slovarnix_ssilok += 1

    dwld_url_gk = f'{slvr_zagr_url[2][1]}'
    r_dwld_url_gk = rq.get(dwld_url_gk, timeout=10, headers=headers)
    with open(f'{slvr_zagr_url[2][0]}', 'wb') as f:
        f.write(r_dwld_url_gk.content)

    dwld_url_tz = f'{slvr_zagr_url[3][1]}'
    r_dwld_url_tz = rq.get(dwld_url_tz, timeout=10, headers=headers)
    with open(f'{slvr_zagr_url[3][0]}', 'wb') as f:
        f.write(r_dwld_url_tz.content)

    print("Выгрузка окончилась успешно! ")
except:
    print("Выгрузка окончилась рядом ошибок, обратитесь с номером данной закупки к моему разработчику. ")
time.sleep(delayer)
print(new_str)
